    ...
    public class ComboBoxWindowTests
    {
        ...
        public void Default()
        {
            using (var app = Application.AttachOrLaunch(ExeFileName, WindowName))
            {
                ...
                {
                    ImageAssert.AreEqual(".\\Images\\Default_ComboBox_Watermark_Win10.png", comboBox);
                }
                else
                {
        ...
        public void DefaultEditable()
        {
            using (var app = Application.AttachOrLaunch(ExeFileName, WindowName))
            {
                ...
                if (WindowsVersion.IsWindows10())
                {
                    ImageAssert.AreEqual(".\\Images\\DefaultEditable_ComboBox_Watermark_Win10.png", comboBox);
                }
                else
                {
                    Assert.Inconclusive($"No image for current windows version.");
